...
// 		return $this->render('index', compact($records)); 
	...
	private function makeCustomer(CustomerRecord $customer_record, PhoneRecord $phone_record){
		...
		$date = new \DateTime($customer_record->birth_date);
		
		$customer = new Customer($name, $date);
		$customer->notes = $customer_record->notes;
		$customer->phones[] = new Phone($customer_record->phone);
		
		return $customer;
	}
	
	private function load(CustomerRecord $customer, PhoneRecord $phone, array $post){
		return $customer->load($post) && $phone->load($post) && $customer->validate() && $phone->validate(['number']);
	}
